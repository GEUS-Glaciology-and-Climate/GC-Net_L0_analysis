...
fn='./ancillary/StationDescription ca.2000 GCN.csv'
...
# for i,fn in enumerate(files[2:]): # just the first station
for i,fn in enumerate(files[12:13]): # all sations
# for i,fn in enumerate(files): # all sations
    ...
    df=df.drop(columns=['SWinMax','SWoutMax','NetRadMax','MaxAirTemp1(TC)','MaxAirTemp2(TC)','MinAirTemp1(TC)','MinAirTemp2(TC)','MaxWindSpeed1','MaxWindSpeed2','StdDevWindSpeed1','StdDevWindSpeed2','RefTemp','WindSpeed@2m','WindSpeed@10m','q1','q2','q3','q4','Albedo'])
    df["date"]=np.nan 
    du=1
    if du:
        for j in range(0,n):
